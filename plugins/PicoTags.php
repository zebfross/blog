...
class PicoTags extends AbstractPicoPlugin
{
    ...
    public function onMetaParsed(&$meta)
    {
		...
		if(array_key_exists('QUERY_STRING', $_SERVER)) {
			parse_str($_SERVER['QUERY_STRING'], $meta['params']);
			$meta['params'] = array_map('urldecode', $meta['params']);	
		}
    }
    ...
    {
        if ($currentPage && !empty($currentPage['meta']['params']['filter'])) {
            $tagsToShow = trim($currentPage['meta']['params']['filter']);

            $pages = array_filter($pages, function ($page) use ($tagsToShow) {
                $tags = PicoTags::parseTags($page['meta']['tags']);
                return in_array($tagsToShow, $tags);
            });
        }
    }
    ...
    {
        $twig->addFilter(new Twig_SimpleFilter('encode_hash', function ($str) {
			return str_replace("#", "%23", urlencode($str));
		}));
    }
}
